...
BEGIN
    ...
    -- 1. Add FEATURED boosts to some listings (Top priority)
    FOR v_listing_id, v_user_id IN 
        SELECT listing_id, user_id FROM listings WHERE status = 'PUBLISHED' LIMIT 5
    LOOP
    ...
    -- 2. Add HOT_BADGE boosts to some other listings (Second priority)
    FOR v_listing_id, v_user_id IN 
    ...
    RAISE NOTICE 'Sample boost data seeded successfully.';
END $$;
