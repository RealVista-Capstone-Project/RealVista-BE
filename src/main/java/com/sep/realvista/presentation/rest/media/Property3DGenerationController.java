...
public class Property3DGenerationController {
    ...
    public ResponseEntity<ApiResponse<Property3DGenerationDto>> initiateOperation(
        ...
        return ResponseEntity.ok(ApiResponse.success("3D operation initiated successfully", result));
    }

    @GetMapping
    @Operation(summary = "Get the list of 3D generation operations and automatically poll status for pending ones")
    public ResponseEntity<ApiResponse<List<Property3DGenerationDto>>> getOperations(
            @PathVariable UUID propertyId) {

        List<Property3DGenerationDto> results = generationService.getOperations(propertyId);
        return ResponseEntity.ok(ApiResponse.success("3D operations retrieved successfully", results));
    }

    @DeleteMapping("/{operationId}")
    @Operation(summary = "Delete a 3D generation operation and its associated media")
    @SecurityRequirement(name = "Bearer Authentication")
    public ResponseEntity<ApiResponse<Void>> deleteOperation(
            @PathVariable UUID propertyId,
            @PathVariable UUID operationId) {

        generationService.deleteOperation(propertyId, operationId);
        return ResponseEntity.ok(ApiResponse.success("3D operation deleted successfully", null));
    }

    @PatchMapping("/{operationId}")
    @Operation(summary = "Update the room name of a 3D generation operation")
    @SecurityRequirement(name = "Bearer Authentication")
    public ResponseEntity<ApiResponse<Property3DGenerationDto>> updateOperation(
            @PathVariable UUID propertyId,
            @PathVariable UUID operationId,
            @Valid @RequestBody UpdateProperty3DOperationRequest request) {

        Property3DGenerationDto result = generationService.updateOperation(propertyId, operationId, request);
        return ResponseEntity.ok(ApiResponse.success("3D operation updated successfully", result));
    }
}
